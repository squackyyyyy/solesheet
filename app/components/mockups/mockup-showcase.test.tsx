...
describe("MockupShowcase", () => {
  it("defaults to the emphasized Quick Sale responsive photograph", () => {
    ...
    expect(screen.getByTestId("planned-flow-figure")).toHaveClass("aspect-[2/3]", "sm:aspect-[4/3]");
  });
  ...
  it("keeps exactly the seven planned-flow selectors operable", () => {
    ...
    for (const label of ["Quick Actions", "Search Stock", "Add Stock", "Installments", "Payments", "Backup"]) {
  ...
  it("switches photographs, descriptions, and source pairs while preserving focus", () => {
    ...
    const image = screen.getByRole("img", { name: /query 530 7 silver resolving across model, size, and colorway/i });
    expect(image).toHaveAttribute("src", "/flow-mockups/search-stock-desktop.webp");
    expect(image.parentElement?.querySelector("source")).toHaveAttribute("srcset", "/flow-mockups/search-stock-mobile.webp");
    expect(screen.getByText("Showing Search Stock product preview")).toHaveAttribute("aria-live", "polite");
  });
