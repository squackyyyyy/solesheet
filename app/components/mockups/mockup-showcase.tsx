...
export function MockupShowcase() {
	...
	const selected =
		flowMockupAssets.find((asset) => asset.id === selectedId) ??
		flowMockupAssets[0];
		...
		<section aria-labelledby="planned-flow-title">
			<div className="grid gap-3 sm:grid-cols-[1fr_.8fr] sm:items-end">
				<div>
					...
					<h3
						id="planned-flow-title"
						className="mt-2 max-w-xl text-2xl font-semibold tracking-tight text-[#171717] sm:text-3xl"
					>
						Seven everyday workflows, shown clearly.
					</h3>
				</div>
				<p className="max-w-md text-xs leading-6 text-black/55 sm:justify-self-end">
					Start with one paid sale: sale recorded → stock updated → profit
					calculated. Then select a moment to switch the preview image.
				</p>
			</div>

			<div
				className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7"
				aria-label="Planned product screen selector"
			>
				{flowMockupAssets.map((asset, index) => {
					...
					return (
						<Button
							...
							aria-label={
								asset.fastestPath ? `${asset.label}, fastest path` : asset.label
							}
						...
						>
							<span
								aria-hidden="true"
								className={`text-[9px] ${isSelected ? "text-white/75" : asset.fastestPath ? "text-emerald-800" : "text-black/55"}`}
							>
								0{index + 1}
							</span>
							<span className="min-w-0">
								...
								{asset.fastestPath ? (
									<span
										className={`mt-0.5 block text-[8px] font-black uppercase tracking-[0.12em] ${isSelected ? "text-[#e8ff9f]" : "text-emerald-800"}`}
									>
										Fastest path
									</span>
								) : null}
