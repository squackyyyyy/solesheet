...
import { surveyQuestions } from "@/app/lib/site-content";
import { isValidWaitlistContact } from "@/app/lib/validation";

type SignupState = "form" | "pending" | "success";
...
export function WaitlistCta({
  ...
  function moveToForm() {
    const contact = document.getElementById("waitlist-contact");
    contact?.focus();
  }

  return (
    <Button variant={variant} onPress={moveToForm} className={className}>
      {children}
      ...
      <div className="grid gap-3">
        ...
        <div className="grid gap-2 rounded-2xl border border-black/10 bg-white p-4 sm:grid-cols-2">
          {surveyQuestions.channels.options.map((channel) => (
            <CheckField
              key={channel}
              isSelected={Array.isArray(answers.channels) && answers.channels.includes(channel)}
...
export function WaitlistExperience() {
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="absolute -bottom-24 -left-20 size-72 rounded-full bg-[#2457ff]/35 blur-3xl" />
      <div className="relative grid gap-10 px-5 py-8 sm:px-8 sm:py-10 lg:grid-cols-[.82fr_1fr] lg:gap-16 lg:px-12 lg:py-14">
        <div className="self-center">
          <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#b9c9ff]">Founding seller list</p>
          <h2 id="waitlist-title" className="mt-4 max-w-xl text-4xl font-semibold tracking-[-0.045em] sm:text-5xl">
            Help build the stockroom you actually want to use.
          </h2>
          <p className="mt-5 max-w-lg text-base leading-7 text-white/62">
            Join early access and tell us which workflow should make the first release. The founding offer is planned at ₱65 per month for the first 50–100 paying sellers.
          </p>
          <div className="mt-8 grid grid-cols-3 gap-3 border-t border-white/12 pt-5 text-xs text-white/70">
            <p><span className="block text-xl font-semibold text-white">20</span>free active pairs</p>
            <p><span className="block text-xl font-semibold text-white">₱99</span>planned Starter</p>
            <p><span className="block text-xl font-semibold text-white">₱65</span>founding rate</p>
          </div>
        </div>

        <div className="rounded-[2rem] bg-[#f8f5ed] p-5 text-[#171717] shadow-[0_30px_100px_rgba(0,0,0,.28)] sm:p-7">
          {signupState === "success" ? (
            <div aria-live="polite" className="flex min-h-[430px] flex-col justify-center text-center">
              <span aria-hidden="true" className="mx-auto grid size-16 place-items-center rounded-full bg-[#e8ff9f] text-2xl">✓</span>
              <p className="mt-6 text-xs font-bold uppercase tracking-[0.24em] text-[#2457ff]">You’re part of the first look</p>
              <h3 className="mx-auto mt-3 max-w-sm text-3xl font-semibold tracking-tight">Thanks, {name.trim() || "seller"}.</h3>
              <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-black/65">
                A few optional questions will help shape the platform, pricing, and workflows we explore next.
              </p>
              <Button onPress={() => setSurveyOpen(true)} className="mx-auto mt-7">
                Answer the quick survey
                <span aria-hidden="true">→</span>
              </Button>
              <Button variant="quiet" onPress={() => setSignupState("form")} className="mx-auto mt-2">
                Back to form
              </Button>
            </div>
          ) : (
            ...
            >
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#2457ff]">Get early access</p>
                <h3 className="mt-2 text-2xl font-semibold tracking-tight">Join the waitlist</h3>
                <p className="mt-2 text-sm leading-6 text-black/65">One quick step now. Product questions come after.</p>
              </div>
              <TextInput
                label="Name"
              ...
              >
                I agree to the collection and use of my information as described in the <a href="#privacy" className="font-semibold underline underline-offset-2">Privacy Notice</a>, including contact about early access.
              </CheckField>
              <Button type="submit" isDisabled={signupState === "pending"} className="mt-1 w-full">
                {signupState === "pending" ? "Joining…" : "Join the waitlist"}
                {signupState !== "pending" ? <span aria-hidden="true">↗</span> : null}
              </Button>
              <p aria-live="polite" className="sr-only">
                {signupState === "pending" ? "Joining the waitlist" : ""}
              </p>
              <p className="text-center text-[11px] leading-5 text-black/65">No payment today. Survey questions are optional.</p>
            </Form>
          )}
        </div>
      </div>
      ...
      <DialogSheet
        title={surveyComplete ? "Thank you for the signal" : "A few quick questions"}
        description={
          surveyComplete
      ...
      >
        {surveyComplete ? (
          <div aria-live="polite" className="grid min-h-80 place-items-center text-center">
            <div>
              <span aria-hidden="true" className="mx-auto grid size-16 place-items-center rounded-full bg-[#e8ff9f] text-2xl">✓</span>
              <h3 className="mt-5 text-2xl font-semibold tracking-tight">That’s the full flow.</h3>
              <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-black/65">Thanks for helping us prioritize speed, clarity, and the features that matter in a real reseller workflow.</p>
              <Button variant="secondary" onPress={() => setSurveyOpen(false)} className="mt-6">Close survey</Button>
            </div>
          </div>
        ) : (
          <SurveyForm answers={answers} setAnswers={setAnswers} onComplete={() => setSurveyComplete(true)} />
        )}
      </DialogSheet>
    </section>
  );
