...
function BrandMark({ compactOnMobile = false, priority = false }: { compactOnMobile?: boolean; priority?: boolean }) {
  return (
    <a href="#top" aria-label="SoleSheet home" className="inline-flex items-center">
...
export default function Home() {
  return (
    ...
    <main id="top" className="overflow-hidden bg-[var(--brand-soft)] text-[var(--brand-ink)]">
      <header className="relative z-30 border-b border-[#14213d]/10 bg-[#f7faf5]/92 backdrop-blur-lg">
        <div className="mx-auto flex h-18 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-10">
          ...
          <nav aria-label="Primary navigation" className="hidden items-center gap-7 text-sm font-medium text-[#14213d]/70 md:flex">
            <a href="#product" className="transition hover:text-[var(--brand-action)]">Product</a>
            <a href="#installments" className="transition hover:text-[var(--brand-action)]">Installments</a>
            <a href="#pricing" className="transition hover:text-[var(--brand-action)]">Pricing</a>
            <a href="#faq" className="transition hover:text-[var(--brand-action)]">FAQ</a>
          </nav>
      ...
      <section className="relative border-b border-[#14213d]/10">
        <div aria-hidden="true" className="absolute -right-52 -top-40 size-[620px] rounded-full border border-[#22c55e]/20" />
        <div aria-hidden="true" className="absolute -right-24 top-8 size-[380px] rounded-full bg-[#dcfce7] blur-3xl" />
        <div className="relative mx-auto grid max-w-[1440px] gap-10 px-4 pb-14 pt-10 sm:px-6 sm:pb-20 sm:pt-14 lg:grid-cols-[minmax(0,.92fr)_minmax(380px,.68fr)] lg:items-center lg:gap-16 lg:px-10 lg:pb-24 lg:pt-18">
          <div className="relative z-10 max-w-3xl">
            ...
            <h1 className="mt-7 text-[clamp(3.15rem,8.8vw,7.5rem)] font-semibold leading-[0.9] tracking-[-0.065em]">
              Your shoe business,
              <span className="mt-1 block font-normal italic text-[var(--brand-action)]">out of the sheet.</span>
            </h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <a href="#product" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[#14213d]/15 bg-white px-5 text-sm font-semibold transition hover:border-[var(--brand-action)] hover:bg-[var(--brand-mist)]">
                Explore the app
          ...
          <div className="relative mx-auto w-full max-w-[430px] lg:justify-self-end">
            <div aria-hidden="true" className="absolute -left-8 top-24 z-10 hidden rotate-[-7deg] rounded-2xl bg-[#e8ff9f] px-4 py-3 text-xs font-bold shadow-xl sm:block">
              Monthly profit<br /><span className="text-xl">₱8,950</span>
            </div>
            <div aria-hidden="true" className="absolute -right-4 bottom-28 z-10 hidden rotate-[6deg] rounded-2xl bg-white px-4 py-3 text-xs font-bold shadow-xl sm:block">
              12 active pairs<br /><span className="font-medium text-black/65">all in one place</span>
            </div>
            <DeviceFrame
          ...
          <div className="grid border-t border-black/12 sm:grid-cols-3 lg:border-t-0">
            {painPoints.map((point) => (
              <article key={point.number} className="border-b border-black/12 py-6 sm:border-b-0 sm:border-l sm:px-5 sm:py-1 first:sm:border-l-0 lg:first:border-l">
                <p className="text-xs font-bold text-[var(--brand-action)]">{point.number}</p>
      ...
      <section id="product" aria-labelledby="product-title" className="border-y border-[#14213d]/10 bg-[var(--brand-mist)]">
        <div className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10">
          <div className="mb-12 grid gap-5 lg:grid-cols-[1fr_.65fr] lg:items-end">
            <div>
              <SectionLabel>See the planned product</SectionLabel>
              <h2 id="product-title" className="mt-5 max-w-3xl text-4xl font-semibold tracking-[-0.05em] sm:text-6xl">
                A quicker way to keep your stockroom current.
              </h2>
            </div>
            <p className="max-w-xl text-sm leading-7 text-black/65 lg:justify-self-end">
              Start with Quick Sale to see how one search, automatic shoe details, and a selling price can turn into a finished log in seconds.
            </p>
          </div>
          <MockupShowcase />
        </div>
      ...
      <section id="installments" aria-labelledby="installment-title" className="relative bg-[var(--brand-ink)] text-white">
        <div aria-hidden="true" className="absolute right-0 top-0 h-full w-1/2 bg-[radial-gradient(circle_at_center,#22c55e44,transparent_65%)]" />
        <div className="relative mx-auto grid max-w-[1440px] gap-12 px-4 py-20 sm:px-6 sm:py-28 lg:grid-cols-[.82fr_1fr] lg:items-center lg:px-10">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.26em] text-[#86efac]">The local differentiator</p>
            ...
            <p className="mt-6 max-w-lg text-base leading-8 text-white/75">
              SoleSheet keeps the stock state and payment state separate, so a released pair can still show exactly what was collected and what remains.
            </p>
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {[
                ["Cash collected", formatPeso(recordedInstallmentPayment.collectedAfter)],
                ["Balance remaining", formatPeso(recordedInstallmentPayment.remainingAfter)],
      ...
      <section id="pricing" aria-labelledby="pricing-title" className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10">
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {plans.map((plan) => (
            <article key={plan.name} className={`relative rounded-[2rem] border p-6 sm:p-7 ${plan.featured ? "border-[var(--brand-action)] bg-[var(--brand-mist)] shadow-[0_24px_70px_rgba(4,120,87,.13)]" : "border-[#14213d]/10 bg-white"}`}>
              {plan.featured ? <span className="absolute right-5 top-5 rounded-full bg-[var(--brand-action)] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-white">Main paid plan</span> : null}
              ...
              <ul className="grid gap-3 text-sm text-black/68">
                {plan.features.map((feature) => <li key={feature} className="flex gap-2"><span className="text-[var(--brand-action)]">✓</span>{feature}</li>)}
              </ul>
            </article>
          ))}
        </div>
      ...
      <section id="faq" aria-labelledby="faq-title" className="border-y border-[#14213d]/10 bg-[var(--brand-mist)]">
      ...
      <footer id="privacy" className="border-t border-black/10">
        <div className="mx-auto grid max-w-[1440px] gap-8 px-4 py-10 text-sm text-black/65 sm:px-6 md:grid-cols-[1fr_auto] md:items-end lg:px-10">
          ...
          <div className="flex flex-wrap gap-5 text-xs font-semibold">
            <a href="#top" className="hover:text-[var(--brand-action)]">Back to top</a>
            <a href="#faq" className="hover:text-[var(--brand-action)]">FAQ</a>
