...
## 2. Wizard-Safe Survey Help

- [x] 2.1 Preserve the one-question-at-a-time survey wizard, temporary answers, current position, and Other-detail behavior while adding active-pair help.
- [x] 2.2 Add an adjacent **What pairs count as active?** link outside the inventory-size field label that opens `/#faq-active-pairs` in a new tab without changing the answer or survey state.
...
- [x] 4.1 Confirm that the current frontend performs no real collection and record operational verification of the final privacy contact, providers, and retention handling as a prerequisite of the future backend collection change.
...
- [x] 5.1 Extend waitlist component tests for the separate active-pair help link, preserved wizard position and answers, Other-detail behavior, and temporary in-session state.
